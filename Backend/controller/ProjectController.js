...
async function createProject(req, res) {
  try {
    ...
    const {name, revenue} = JSON.parse(body);

    const newId = projects[projects.length-1].id + 1;

 
    projects.push({ id: newId, name: name, revenue: revenue, isCompleted: false });

    sendResponse(res, 200,{ id: newId, name: name, revenue: revenue, isCompleted: false });
  } catch (error) {
    console.log(error);
  }
}
